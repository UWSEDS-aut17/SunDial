...
def clean_weather_data(data):
    ...
    """
    {'Windy and Partly Cloudy': 1, 'Mostly Cloudy': 2, 'Breezy and Mostly Cloudy': 3, 'Breezy and Partly Cloudy': 4, 'Partly Cloudy': 12,
    'Clear': 5, 'Overcast': 6, 'Light Rain': 7, 'Possible Light Rain': 11, 'Rain': 9, 'Breezy': 10, 'Windy': 8, 'Heavy Rain': 0, 'Windy and
    Mostly Cloudy': 13, 'Foggy': 14, 'Breezy and Overcast': 15, 'Light Rain and Breezy': 16, 'Rain and Breezy': 17}
    """
    summary_values = list(set(subset["summary"]))
    summary_dict = {value:i for (i, value) in enumerate(summary_values)}
    subset = subset.replace({"summary" : summary_dict}).drop(subset.columns[[0, 1,10,14]],
                                                             axis=1).fillna(0.0)
    subset = subset.reindex_axis(['time','Date','year','month','day', 'Hour','summary','precipIntensity','precipProbability',
                              'temperature','apparentTemperature','dewPoint','humidity','windSpeed','windBearing'
...
def get_data(solar):
    ...
    for i in range(solar.shape[0]):
        B = (((solar[i,6]*((np.sin(decl)*np.sin(lat)*np.cos(tilt))-(np.sin(decl)*np.cos(lat)*np.sin(tilt)*np.cos(azi))
                        +(np.cos(decl)*np.cos(lat)*np.cos(tilt))+(np.cos(decl)*np.sin(lat)*np.sin(tilt)*np.cos(azi))
                        +(np.cos(decl)*np.sin(tilt)*np.sin(azi)))) + ((solar[i, 5])*((1.14159-tilt)/(3.14159))))*eff*3.6)
        elec.append(B)
...
def get_data_split(year3, month3, day3):
    ...
    train = pv_output_array[(pv_output_array[:,1 ] >= (date_train_start)) & (pv_output_array[:,1 ] <= date_train_stop) &
                            (pv_output_array[:,4] >= 0)]
    test = pv_output_array[(pv_output_array[:,1 ] >= (date_test_start)) & (pv_output_array[:,1 ] <= date_test_stop) &
                           (pv_output_array[:,4] >= 0)]
...
def SVM_regressor():
    ...
    # In a perfect world, we could have enough time to optimize.
