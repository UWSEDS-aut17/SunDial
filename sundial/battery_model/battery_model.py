...
# First, train storage model (battery degradation from simply sitting, not from
# cycling
...
seed = 7
X2_train, X2_validation, Y_train, Y_validation = \
    model_selection.train_test_split(X2, Y, test_size=validation_size,
                                     random_state=seed)
...
def bat_shelf(soc, temp, days):
    """Model the degradation from the battery sitting there (not from cycling).
    inputs are state of charge (soc,%, 0 - 100) which is the state of charge
    while resting, temperature (temp, C, -40 - 50 C), and days (length of time
    for case, trained up to 150 days, but we will use 30 days then extrapolate
    degradation to adjust for daily calls)
    """
...
def bat_cycle(cycles, soc_low, soc_high, rate):
    """Model the degradation from the battery due to cycles. inputs are cycle
    #(1-800), lower limit state of charge of cycle (soc,%, 0 - 40) which is the
    tate of charge prior to charging, upper limit state of charge of
    cycle (soc,%, 60 - 100) which is the state of charge prior to discharging,
    and the discharging rate (0.5C - 2C)
    """
    # Maybe add some code to check inputs are OK range, etc.
...
def bat_price_per_hour(energy, hour_start, hour_end, day, bat_cap, bat_cost):
    """This function outputs a 24x1 vector with the cost of using the battery
    for given inputs. The basic ideal is to call the degradation model, then
    multiply the very little degradation in each hour by the total cost of the
    battery.
    ...
    if cycles == 0:
        ...
        deg_hour[hour_start:hour_end] = deg_hour[hour_start:hour_end] * \
            deg_cycle_hour
