...
data2 = PL03{3,3};
%}
load('data/SOC_20%-80%_HalfC/PL21.mat')
...
for jj = [5:14]
    ...
    cap2 = diff(cap);
    dataset(idx1:idx1+length(cap2)-1,1) = day(datetime(PL21{jj,2}) - datetime(PL21{2,2})); %fix year thing
    dataset(idx1:idx1+length(cap2)-1,2) = 0.5; %always
    dataset(idx1:idx1+length(cap2)-1,3) = 0.5;
    dataset(idx1:idx1+length(cap2)-1,4) = 0;
    dataset(idx1:idx1+length(cap2)-1,5) = 60;
...
dataset(find(dataset(:,7) > 1.1),:) = [];
dataset(find(dataset(:,7) < .5),:) = [];
toc
